...
export function PGTile({ pg, onClick, badge }: { pg: PG; onClick: () => void; badge?: string }) {
  ...
  return (
    <div className="group relative h-full">
      <button
        onClick={onClick}
        className="flex h-full w-full flex-col gap-2.5 rounded-xl border border-border bg-card p-3.5 text-left shadow-card transition-smooth hover:border-primary/40 hover:shadow-glow relative"
      >
        {badge && (
          <div className="absolute -top-2 left-3 z-10 rounded-full border border-primary/40 bg-primary/10 px-2 py-0.5 text-[10px] font-mono text-primary shadow-sm">
            {badge}
          </div>
        )}
        ...
        <div className="flex items-start justify-between gap-3 w-full">
          <div className="min-w-0 flex-1 pr-7">
            <div className="font-display text-base font-semibold leading-tight truncate group-hover:text-primary transition-colors">{pg.name}</div>
          ...
          <div className="flex flex-col items-end gap-0.5 shrink-0 pr-8">
            <div className={cn("font-mono text-2xl font-bold leading-none tabular-nums tracking-tight", iqColor(pg.iq))}>
              {pg.iq}
            </div>
            <div className="text-[9px] uppercase tracking-widest text-muted-foreground font-semibold">IQ Score</div>
          </div>
        </div>

        {/* Persona pitch badge */}
        <div className={cn("flex items-center gap-1.5 rounded-md border px-2 py-1 text-[10px] font-medium w-fit", pStyle.color)}>
        ...
        {closest && (
          <div className="flex items-center gap-1.5 rounded-md bg-surface-2 px-2 py-1.5 text-xs">
            ...
            <span className="truncate flex-1 font-medium">{closest.n}</span>
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-auto pt-1">
          <span className={cn("rounded-md border px-2 py-0.5 text-[10px] font-medium tracking-wide", tierColor(pg.tier))}>{pg.tier}</span>
          <span className={cn("rounded-md px-2 py-0.5 text-[10px] font-medium tracking-wide", genderColor(pg.gender))}>{pg.gender}</span>
        ...
        <div className="flex items-end justify-between border-t border-border pt-2.5 mt-1 w-full">
          <div>
            <div className="font-mono text-sm tabular-nums font-medium">{price.primary}</div>
            {price.perDay && (
              <div className="font-mono text-[10px] text-emerald-400 tabular-nums leading-none mt-0.5 font-medium">
                {price.perDay}
              </div>
            )}
          </div>
          {sc.hot ? (
            <span className="inline-flex items-center gap-1 rounded-md border border-rose-400/40 bg-rose-400/10 px-1.5 py-0.5 text-[10px] font-bold text-rose-400 animate-pulse-dot">
              <Flame className="h-3 w-3" /> {sc.level}
            </span>
          ) : sc.level === "FEW LEFT" ? (
            <span className="inline-flex items-center gap-1 rounded-md border border-amber-400/40 bg-amber-400/10 px-1.5 py-0.5 text-[10px] font-bold text-amber-400">
              {sc.level}
            </span>
          ) : sc.level === "FULL" ? (
      ...
      <button
        ...
        className={cn(
          ...
          saved ? "text-amber-400 bg-amber-400/10 hover:bg-amber-400/20" : "text-muted-foreground opacity-0 group-hover:opacity-100 hover:bg-surface-2"
        )}
      >
